...
# keep track of errors that should cause unbound not to start
UB_LIST_ERRORS=""
...
# lan/wan_iface list used in config
UB_B_LAN_SPECIFIED=0
UB_B_WAN_SPECIFIED=0
...
bundle_lan_networks() {
  local interface="$1"
  local ifsubnet ifname ifdashname ipaddrs

  UB_B_LAN_SPECIFIED=1

  network_get_device ifname "$interface"

  if [ -n "$ifname" ] ; then
    network_get_ipaddrs_all ipaddrs "$interface"
    UB_LIST_LAN="$UB_LIST_LAN$interface|$ifname|$ipaddrs
"
  fi
...
bundle_wan_networks() {
  ...
  UB_B_WAN_SPECIFIED=1

  network_get_device ifname "$interface"

  if [ -n "$ifname" ] ; then
    network_get_ipaddrs_all ipaddrs "$interface"
    UB_LIST_WAN="$UB_LIST_WAN$interface|$ifname|$ipaddrs
"
  fi
...
unbound_conf() {
  ...
  local interface ifname ipaddrs ipaddr wan_addr_found
  ...
  if [ $UB_B_IF_AUTO -gt 0 ] ; then
    ...
    {
      if [ -z "$UB_LIST_LAN" ] ; then
        UB_LIST_ERRORS="${UB_LIST_ERRORS}no lan device found
"
      else
        while IFS='|' read -r interface ifname ipaddrs ; do
          echo "  interface: $ifname  # $interface: $ipaddrs" ;
        done < <(echo -n "$UB_LIST_LAN")
      fi
      ...
      while IFS='|' read -r interface ifname ipaddrs ; do
        ...
        for ipaddr in $ipaddrs ; do
          echo "  outgoing-interface: $ipaddr"
          wan_addr_found=1
        done
      done < <(echo -n "$UB_LIST_WAN")

      if [ $UB_B_WAN_SPECIFIED -gt 0 ] && [ $wan_addr_found -eq 0 ] ; then
        UB_LIST_ERRORS="${UB_LIST_ERRORS}no wan IP address found
"
      fi
    } >> $UB_CORE_CONF
  fi
...
unbound_uci() {
  local cfg="$1"
  local hostnm interface ifname ipaddrs
  ...
  if [ $UB_B_LAN_SPECIFIED -eq 0 ] ; then
    for interface in loopback lan ; do
      network_get_device ifname "$interface"
...
resolv_setup() {
  if [ "$UB_N_RX_PORT" != "53" ] ; then
    ...
    {
      echo "# $UB_RESOLV_CONF generated by Unbound UCI"
...
unbound_start() {
  ...
  if [ $UB_B_MAN_CONF -eq 0 ] ; then
    ...
    # merge
    unbound_include
    # break config and make unbound quit on start on error
    while read -r error ; do
      logger -p err -t unbound -s "$error"
      echo "ERROR: $error" >> $UB_TOTAL_CONF
    done < <(echo -n "$UB_LIST_ERRORS")
  fi
